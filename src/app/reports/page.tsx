...
export default async function ReportsPage() {
  await requireUser();
  const [total, won, lost, killed, sources, stages] = await Promise.all([
    db.lead.count(),
  ...
  return (
    <CrmShell title="Reports" eyebrow="CRM performance">
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {[["Total leads", total], ["Won", won], ["Lost", lost], ["Killed", killed]].map(([label, value]) => (
      ...
      <section className="mt-6 grid gap-6 lg:grid-cols-2">
        ...
        <article className="rounded-xl border border-[#e2e8e5] bg-white p-6 shadow-sm">
          ...
          <div className="mt-5 space-y-3">
            {stages.map((stage) => <div key={stage.stage} className="flex items-center justify-between border-b border-[#edf0ef] pb-3 text-sm"><span className={`rounded-full border px-2.5 py-1 text-xs font-bold ${leadStageBadgeClass(stage.stage)}`}>{leadStageLabel(stage.stage)}</span><b>{stage._count._all}</b></div>)}
          </div>
        </article>
      </section>
    </CrmShell>
